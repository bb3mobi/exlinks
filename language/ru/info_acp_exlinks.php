...
$lang = array_merge($lang, array(
	...
	'ACP_HIDE_LINKS_GUESTS_EXPLAIN'	=> 'Введите текст и названия ссылок будут заменены им.',
	'ACP_EXTERNAL_LINK_PREFIX'		=> 'Префикс перед ссылкой',
	'ACP_EXTERNAL_LINK_REDIRECT'	=> 'Время для перенаправления',
	'ACP_EXTERNAL_LINK_EXACT'		=> 'Точная ссылка:',

	'ACP_INTERNAL_LINK_DOMAINS'		=> 'Список доменов, на которые не будут распространяться действия выше, ипользуйте ";" (точку с запятой для разделения)',
	'ACP_FORBIDDEN_DOMAINS'			=> 'Список доменов, ссылки которых не будут отображаться, используйте ";" (точку с запятой для разделения)',
...
/* New lang */
	...
	'MESSAGE_TEXT'	=> '<strong>Вы переходите по внешней ссылке: <span class="error">%s</span></strong><p>Убедитесь, что данная ссылка полностью является доверенной и ограждена от вредоносных влияний.<br />Если же ссылка показалась вам подозрительной, уважительная просьба сообщить об этом администрации.</p>',
	'URL_LINK'		=> '%sПерейти по указанной ссылке%s',
	'CLOSE_PAGE'	=> '%sЗакрыть и вернуться на страницу%s',
));
